...
\midrule
Dương Trung Hiếu (Web/Local GUI) & Built the interface, map, route selection, traversal animations, route explanations and metrics, and algorithm comparison; implemented multi-location route optimization; and supported backend integration.\\
Trần Vũ Anh Kiệt (Core Algorithms) & Designed the graph structure; implemented BFS, DFS, UCS, A*, and advanced single-route search; and optimized and debugged the algorithms.\\
...
Ho Chi Minh City. A user chooses a start location, a destination, optional
...
\titleformat{\subsection}{\large\bfseries\color{hcmusblue}}{\thesubsection}{0.7em}{}
\subsection{Nodes}
Each node represents a location or intersection in the road network. A node has
the following attributes:
\begin{itemize}
\item \code{node\_id}: the unique identifier of the node, derived from the key
of the corresponding object in the JSON dataset.
\item \code{name}: the human-readable street or intersection name displayed in
the user interface.
\item \code{lat}: the latitude of the location.
\item \code{lng}: the longitude of the location.
\item \code{type}: the node category, such as \code{intersection}.
\end{itemize}

\subsection{Edges}
Each edge represents a directed road segment from one node to another. An edge
has the following attributes:
\begin{itemize}
\item \code{source\_id}: the identifier of the source node, derived from the
node that owns the \code{connected\_to} list.
\item \code{target\_node}: the identifier of the destination node.
\item \code{distance}: the physical length of the road segment in metres.
\item \code{estimated\_time}: the nominal travel time in seconds under normal
traffic conditions.
\item \code{congestion\_level}: the simulated traffic level from 1 (smooth) to
5 (gridlock).
\item \code{road\_type}: the road classification, for example
\code{primary}, \code{secondary}, or \code{residential}.
\item \code{direction}: whether the road is \code{one-way} or \code{two-way}.
\item \code{risk\_factors}: a list of possible risks, such as flooding,
construction, narrow roads, or complex intersections.
\item \code{geometry}: the ordered latitude--longitude points used to draw the
road segment on the map.
\end{itemize}

The graph stores these directed edges in an adjacency list. This representation
supports efficient neighbor lookup and uses $O(|V|+|E|)$ memory.
\titleformat{\subsection}{\large\bfseries\color{darkgray}}{\thesubsection}{0.7em}{}
...
vertex 2 (An Dương Vương - Đường Hồng Bàng) in mixed-cost mode. The results
recorded from the graphical interface are summarized in Table~\ref{tab:actual}.
Because this is a very short route, the measured execution times are mainly
illustrative; route quality and the number of explored vertices are more
meaningful for comparison.

\begin{table}[H]
\centering
\caption{Actual mixed-mode results from vertex 0 to vertex 2}
\label{tab:actual}
\small
\begin{tabular}{lrrrrr}
\toprule
\textbf{Method}&\textbf{Dist. (m)}&\textbf{Time (min)}&
\textbf{Cost}&\textbf{Explored}&\textbf{ms}\\
\midrule
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{images/route-comparison-0-to-2.png}
\caption{GUI results for the route from Đường Hồng Bàng - Đường Đặng Thái
Thân to An Dương Vương - Đường Hồng Bàng: (a) A* in mixed mode, (b) BFS,
(c) DFS, (d) UCS, (e) Dijkstra, and (f) Greedy Best-First in mixed mode.}
\label{fig:route-comparison-0-to-2}
\end{figure}

\paragraph{Conclusion.}
A*, BFS, UCS, Dijkstra, and Greedy Best-First all find the same 128-metre route,
with an estimated travel time of 1.1 minutes and a mixed cost of 383.92. A* and
Greedy Best-First are the most search-efficient in this experiment, exploring
only two vertices, while BFS has the lowest measured execution time at
0.08~ms. DFS is strongly affected by adjacency order: it explores four
vertices and returns a 296-metre detour taking 4.6 minutes, with a cost of
1,564.24. Therefore, the informed and cost-based methods provide substantially
better route quality than DFS for this query; the tiny timing differences are
not large enough to establish a general runtime ranking.
