...
directed road graph in Ho Chi Minh City. The dataset contains 212 geographic
vertices and 1,129 directed adjacency entries after two-way roads are expanded.
...
Greedy Best-First Search for two-location routing, together with prototype
...
Experiments on a reachable 28-hop test case show that UCS, Dijkstra, and A*
produce the same minimum distance-mode cost. A* expands 158 vertices instead of
198 for UCS, while Greedy Best-First expands only 34 but returns a route whose
cost is 6.1 percent higher. In a five-location cycle, Held-Karp reduces mixed
cost by 11.32 percent relative to the supplied visit order and by 26.04 percent
relative to Nearest Neighbor. The report also records integration gaps found in
the current repository so that completion claims remain reproducible.
...
on the repository state inspected on August 15, 2026.
...
\midrule
1 & Nguyễn Trường Sơn & 24127233\\
2 & Lê Nguyễn Quốc Học & 24127044\\
3 & Nguyễn Hoàng Bảo Long & 24127201\\
4 & Trần Vũ Ánh Kiệt & 24127196\\
5 & Tạ Văn Đức & 23127528\\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Nguyễn Trường Sơn & Problem analysis, experiment design, test-case review, and report coordination.\\
Lê Nguyễn Quốc Học & DFS/BFS principles, algorithm comparison, validation cases, and presentation preparation.\\
Nguyễn Hoàng Bảo Long & Traffic-data generation and cleaning, geographic node/edge preparation, and map visualization.\\
Trần Vũ Ánh Kiệt & Cost evaluator, informed search algorithms, FastAPI schemas/endpoints, and backend integration.\\
Tạ Văn Đức & React interface, interaction controls, frontend packaging, Docker/Nginx setup, and demo support.\\
\bottomrule
\end{tabularx}
\end{table}

\section{Requirement Completion Level}
Table~\ref{tab:completion} distinguishes implemented functions from prototypes
and integration work still required. This avoids treating source-code presence
as proof of end-to-end completion.
...
GUI and graph display & \partialstatus & Map and controls render; current search URL/label mapping needs alignment with the API.\\
Step-by-step visualization & \partialstatus & Playback controls exist, but the displayed step counter is not yet driven by the returned exploration list.\\
Multi-location optimization & \partialstatus & Nearest Neighbor and Held-Karp exist; an obsolete A* keyword prevents direct execution without a compatibility wrapper.\\
Route explanation & \partialstatus & An explanation card exists, but its text is generic rather than generated from route segments.\\
Automated testing & \partialstatus & Smoke tests run; the legacy runner uses an obsolete cost API and the current default pair can be unreachable.\\
\bottomrule
\end{tabularx}
\end{table}
...
d_{\mathrm{eff}}(e)+10t_{\mathrm{eff}}(e),&\text{mixed mode}.
\end{cases}
\label{eq:cost}
\end{equation}
...
source JSON contains 212 nodes and road-segment records. When two-way segments
are expanded in memory, the adjacency lists contain 1,129 directed edges. This
exceeds the minimum Lab 1 requirement of 20 nodes and 30 edges.

\section{Representative Locations}
The complete location list is stored in \code{data/nodes.json}. Representative
...
65 & Ngã giao Đường Tản Đà - Đường Trần Hưng Đạo & Required stop\\
...
The largest strongly connected component contains 200 of the 212 vertices. The
remaining directed structure explains why arbitrary first/last JSON entries may
produce a no-path result. Experiments therefore use explicitly verified pairs.
The data is suitable for algorithm comparison but does not claim citywide map
...
src/algorithms/\newline uninformed.py & DFS and BFS.\\
...
The frontend imports graph data for immediate map display. A search request is
intended to send start, end, algorithm, and optimization to FastAPI. The backend
selects a \code{CostEvaluator}, dispatches the algorithm, and returns path IDs,
coordinates, names, exploration order, and metrics. The current branch must
align \code{/search} with \code{/api/search} and normalize labels such as
\code{A*} to the backend value before this flow is fully end-to-end.
...
The verified query is vertex 123 (Đường Nguyễn Văn Cừ) to vertex 151 (Ngã giao
Đường Nguyễn Trãi - Đường Phú Giáo) in distance mode. One local Python 3.13 run
...
\begin{table}[H]
\centering
\caption{Actual distance-mode results from vertex 123 to 151}
\label{tab:actual}
\small
...
UCS, Dijkstra, and A* agree on the optimum. A* reduces expansions by 20.2
percent relative to UCS. Greedy expands only 34 vertices but costs 6.1 percent
...
Time mode accepts approximately 252 additional metres to reduce adjusted time
by 277.5 seconds (7.6 percent). This is direct evidence that congestion changes
...
The depot is vertex 125 and the supplied visit order is
$65\rightarrow192\rightarrow155\rightarrow81$. Mixed mode is used. Because the
current multi-location module passes an obsolete \code{heuristic\_type} keyword,
the experiment used a compatibility wrapper that discards this keyword and
calls the current A* implementation. No route costs were otherwise changed.
...
Held-Karp improves the original order by 11.32 percent and Nearest Neighbor by
26.04 percent. Nearest Neighbor is worse than even the original order in this
case, illustrating that locally cheapest decisions do not guarantee a good
cycle. Held-Karp is optimal for the five selected locations under the computed
pairwise mixed costs, but its exponential growth limits it to small stop sets.

\section{Integration Status}
The algorithms and reconstruction logic exist in
\code{src/algorithms/multi\_location.py}, but they are not dispatched by the API
or displayed by the frontend. The obsolete keyword call must be removed, then
API schemas should accept a stop list and return visiting order, detailed path,
and cost before this requirement is considered end-to-end complete.
...
Open \url{http://localhost:5173}. Before end-to-end use on the inspected branch,
change the frontend request from \code{/search} to \code{/api/search} and align
algorithm labels with the backend map. These are integration fixes, not extra
...
\item Choose Default, Time, Distance, or Cost optimization.
\item Choose DFS, BFS, Dijkstra, or A* from the current GUI list.
...
\caption{Current React interface displaying the 212-node road graph}
\label{fig:gui}
\end{figure}
...
and coordinates. For this example, A* returns 31 path vertices, 4,781.20 metres,
cost 5,442.72, and 158 expanded vertices in the recorded run.
...
but its cost is 8.0 percent higher. The time-mode alternative is physically
longer, yet its adjusted time is 277.5 seconds lower because it avoids more
...
\item Frontend and backend route paths/algorithm labels are misaligned on the inspected branch.
\item The frontend overwrites the returned path with a temporary direct waypoint list.
\item Playback controls are not yet linked to real frontier/exploration states.
\item Route explanation text is generic and does not identify congested edges or compare alternatives dynamically.
\item Multi-location methods are not exposed through the API and contain one obsolete function argument.
\item Automated tests do not yet cover API contracts, optimality, disconnected graphs, or repeated benchmarks.
\end{itemize}

\section{Future Work}
The highest-priority work is to align the API path and algorithm naming, remove
the temporary frontend path override, synchronize the multi-location call, and
add contract tests. The exploration list should drive the visualizer, while the
explanation generator should cite actual high-congestion and risk edges.
...
However, the documented integration gaps must be closed before the GUI,
step-by-step explanation, and multi-location requirements can be considered
fully complete.
...
\url{https://hcmus.edu.vn/he-thong-nhan-dien-thuong-hieu/}, accessed August 15, 2026.

\bibitem{fastapi}
FastAPI, \textit{FastAPI Documentation},
\url{https://fastapi.tiangolo.com/}, accessed August 15, 2026.

\bibitem{react}
Meta Open Source, \textit{React Documentation},
\url{https://react.dev/}, accessed August 15, 2026.
\end{thebibliography}

\appendix
\chapter{Repository Structure}
\begin{verbatim}
data/                          Road graph JSON files
src/models.py                  Graph and cost evaluator
src/heuristics.py              Haversine/time heuristics
src/algorithms/base.py         Common search types
src/algorithms/uninformed.py   DFS and BFS
src/algorithms/informed.py     UCS, Dijkstra, A*, Greedy
...
\begin{quote}\small
123 $\rightarrow$ 172 $\rightarrow$ 171 $\rightarrow$ 137 $\rightarrow$ 211
$\rightarrow$ 47 $\rightarrow$ 105 $\rightarrow$ 200 $\rightarrow$ 210
$\rightarrow$ 98 $\rightarrow$ 208 $\rightarrow$ 112 $\rightarrow$ 42
$\rightarrow$ 40 $\rightarrow$ 41 $\rightarrow$ 80 $\rightarrow$ 65
$\rightarrow$ 66 $\rightarrow$ 99 $\rightarrow$ 146 $\rightarrow$ 25
$\rightarrow$ 72 $\rightarrow$ 74 $\rightarrow$ 78 $\rightarrow$ 45
$\rightarrow$ 75 $\rightarrow$ 58 $\rightarrow$ 101 $\rightarrow$ 181
$\rightarrow$ 182 $\rightarrow$ 151.
\end{quote}
